...
class Test:
    ...
    def run(self):
        ...
        if output.split() == open(self.output).read().split():
            print("✅ Test " + str(path.basename(self.input).split(".")[0]) + " passed. Runtime: " + str(
                round(runTime, 10)) + " seconds.")

            return True
        else:
            print("❌ Test " + str(path.basename(self.input).split(".")[0]) + " failed. Runtime: " + str(
                round(runTime, 10)) + " seconds.")
            print("\tExpected output: \n", end="")
            print("\t"+open(self.output).read())
            print("\tActual output:\n",end="")
            print('\t'+str(output))
            return False
    ...
    def runWithoutResult(self):
        ...
        timeEnded = time.time()
        runTime = timeEnded - timeStarted
        print("🏁 Test " + str(path.basename(self.input).split(".")[0]) + " finished. Runtime: " + str(round(runTime, 10))
              + " seconds.")
        return output
